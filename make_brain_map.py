...
def attach_colour_groups(tagged: ddf.DataFrame):
    ...
    colour_series = tagged.map_partitions(
...
def save(plotter, outdir):
    ...
    plotter.screenshot(os.path.join(outdir, "brain_map_above.png"))
    plotter.view_vector((1, 0, 0)) # Side / looking along x-axis
    plotter.screenshot(os.path.join(outdir, "brain_map_side.png"))
    plotter.view_vector((0, 1, 0)) # Front / looking along y-axis
    plotter.screenshot(os.path.join(outdir, "brain_map_front.png"))    


def prepare_plotter(tagged: ddf.DataFrame):
